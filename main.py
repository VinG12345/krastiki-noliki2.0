...
class TicTacToeApp:
    ...
    def _setup_ui(self):
        ...
        tk.Label(settings_frame, text="|", bg="#16213e", fg="#555").pack(
            side="left", padx=10
        )
    ...
    def _minimax(self, board, is_maximizing, depth):
        if self._check_winner_board(board, self.ai_player):
            return 10 - depth
        if self._check_winner_board(board, self.human_player):
            return depth - 10
        if self._is_board_full_board(board):
            return 0
    ...
    def _ai_move(self):
        ...
        print(f"[AI] Move selected: {best_move}")
        if best_move:
            r, c = best_move
    ...
    def _check_winner_ui(self, player):
        b = self.buttons
        for i in range(3):
            if b[i][0]["text"] == b[i][1]["text"] == b[i][2]["text"] == player:
                return True
            if b[0][i]["text"] == b[1][i]["text"] == b[2][i]["text"] == player:
                return True
        if b[0][0]["text"] == b[1][1]["text"] == b[2][2]["text"] == player:
            return True
        if b[0][2]["text"] == b[1][1]["text"] == b[2][0]["text"] == player:
            return True
        return False
    ...
    def _round_end(self, winner):
        ...
        if self.scores[winner] >= self.MATCH_TARGET:
            ...
            self._unlock_controls()
            self._update_status(
                "Раунд окончен. Выбери сторону и нажми ▶️ СТАРТ", "#00c853"
            )
